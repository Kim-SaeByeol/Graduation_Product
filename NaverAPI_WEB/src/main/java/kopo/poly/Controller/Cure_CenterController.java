...
public class Cure_CenterController {
    private final ICure_CenterService centerService;

    @GetMapping(value = "Cure_centerReg")
    public String Cure_centerReg(HttpServletRequest request, HttpSession session, ModelMap model) {
        log.info(this.getClass().getName() + ".Cure_centerReg에 들어왔음.");
        /**
         * centerReg.html 파일에 selectedInsertType에 값을 넣어 보내
         *  하나의 html 로 3개의 Center들의 Insert 를 담당함.
         **/
        model.addAttribute("selectedInsertType", "Cure");
        return "/center/centerReg";
    }

    @ResponseBody
    @PostMapping(value = "Cure_centerInsert")
